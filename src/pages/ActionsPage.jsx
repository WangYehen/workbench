...
import EmailsPage from "./EmailsPage.jsx";
...
import ManagementQuestions from "../components/ManagementQuestions.jsx";
...
export default function ActionsPage() {
  ...
  return <div className="workspace-page">
    <header className="workspace-head"><div><h1>行动中心</h1><p>从发现问题到分类、处理、转待办和跟踪的统一入口</p></div>{tab === "attention" && <DateNav date={date} onChange={setDate}/>} {tab === "email" && <div className="actions-mail-sync"><span><i />最近成功 {lastSuccess}</span><SyncButton className="btn primary" type="button" syncing={emailSyncing} disabled={!syncEmailRef.current} onClick={() => syncEmailRef.current?.()}>立即同步</SyncButton></div>}{tab === "dingtalk" && <div className="actions-mail-sync"><span><i />最近成功 {dingtalkLastSuccess}</span><SyncButton className="btn primary" type="button" syncing={dingtalkSyncing} disabled={!syncDingtalkRef.current} onClick={() => syncDingtalkRef.current?.()}>立即同步并研判</SyncButton></div>}</header>
    <nav className="workspace-tabs" aria-label="行动中心视图">{[["attention","管理事项"],["email","邮件"],["dingtalk","钉钉消息"],["tasks","待办"]].map(([key,label])=><button key={key} className={tab===key?"is-active":""} onClick={()=>setTab(key)}>{label}</button>)}</nav>
    {error && <div className="error">{error}</div>}
    {tab === "email" && <EmailsPage embedded onStatusChange={setOutlookStatus} onSyncReady={setEmailSync} onSyncingChange={setEmailSyncing} />}
    {tab === "dingtalk" && <DingtalkMessagesPage onStatusChange={setDingtalkStatus} onSyncReady={setDingtalkSync} onSyncingChange={setDingtalkSyncing} />}
    {tab === "tasks" && <TodosPage embedded />}
    {tab === "attention" && <><ManagementQuestions items={items} onOpen={(item) => document.getElementById(`attention-${item.id}`)?.scrollIntoView({ behavior: "smooth", block: "center" })} /><div className="attention-list" style={{ marginTop: 18 }}>{items.map((item)=><article id={`attention-${item.id}`} className="attention-item" key={item.id}><PriorityBadge priority={item.priority}/><div><h3>{item.title}</h3><p>{item.detail || ""}</p></div><span className="attention-item__action">{item.recommendedAction}</span></article>)}{!items.length&&<div className="empty-state">该日期没有待处理管理事项。</div>}</div></>}
  </div>;
}
