...
const CLOSED_STATUSES = ["processed", "ignored", "converted"];
...
function closedStatusLabel(status) {
  if (status === "processed") return "处理完成";
  if (status === "ignored") return "标记无需处理";
  if (status === "converted") return "已转待办";
  return "已关闭";
}

function closedAtOf(message) {
  if (!message?.updatedAt) return null;
  const ts = new Date(message.updatedAt);
  return Number.isNaN(ts.getTime()) ? null : ts;
}

function dueInfo(message) {
  // 已关闭（已处理/无需处理/已转待办）的邮件不再以"已逾期"等风险口径呈现，
  // 避免关闭状态与风险状态互相冲突。
  if (isClosedStatus(message.status)) {
    const closedAt = closedAtOf(message);
    return {
      tone: "archived",
      label: "已归档",
      detail: closedAt
        ? closedAt.toLocaleString("zh-CN", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })
        : "—",
    };
  }
...
export default function EmailsPage({ embedded = false, onStatusChange, onSyncReady, onSyncingChange }) {
  ...
  const visible = useMemo(() => {
    ...
    return [...(data[view] || [])].filter((m) => !keyword || [m.sender, m.subject, m.summary, m.actionText].some((x) => String(x || "").toLowerCase().includes(keyword))).sort((a, b) => {
      // 已归档邮件按"处理时间"或"接收时间"倒序，不再按截止时间排序
      if (isClosedStatus(a.status) || isClosedStatus(b.status)) {
        const aKey = closedAtOf(a)?.getTime() ?? Date.parse(a.receivedAt || 0) ?? 0;
        const bKey = closedAtOf(b)?.getTime() ?? Date.parse(b.receivedAt || 0) ?? 0;
        return bKey - aKey;
      }
      const rank = { overdue: 0, today: 1, later: 2, none: 3, archived: 4 };
  ...
  return <div className="workspace-page m2">
    ...
    <div className="m2-workspace"><section className="m2-queue"><div className="m2-search"><IconSearch size={18}/><input value={search} onChange={(e) => { setSearch(e.target.value); setPageByView({ action: 1, informational: 1, uncertain: 1, archive: 1 }); }} placeholder="搜索邮件" aria-label="搜索邮件" /></div><div className="m2-tabs">{Object.entries(queues).map(([key, item]) => <button key={key} onClick={() => setView(key)} className={view === key ? "is-active" : ""}>{item.label}<span>{totals[key]}</span></button>)}</div><div className="m2-list-head"><span>优先级</span><span>发件人</span><span>邮件主题</span><span>时间 / 截止</span></div><div className="m2-list">{visible.map((m) => { const due = dueInfo(m); const archived = isClosedStatus(m.status); const closedAt = archived ? closedAtOf(m) : null; return <button key={m.id} className={`m2-row ${selected?.id === m.id ? "is-selected" : ""} ${archived ? "m2-row--archive" : ""}`} onClick={() => setSelectedId(m.id)}><span className={`m2-priority ${m.priority ? `m2-priority--${m.priority.toLowerCase()}` : ""} ${archived ? "m2-priority--archived" : ""}`}>{m.priority || "—"}</span><span className="m2-row__sender">{m.sender || "未知发件人"}</span><span className="m2-row__subject">{m.subject || "（无主题）"}</span><span className={`m2-row__due m2-row__due--${due.tone}`}><b>{due.tone === "overdue" ? <IconAlertTriangle size={14}/> : due.tone === "today" ? <IconClock size={14}/> : due.tone === "archived" ? <IconArchive size={14}/> : null}{due.label}</b><small>{archived ? `${closedStatusLabel(m.status)}${closedAt ? ` · ${closedAt.toLocaleString("zh-CN", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}` : ""}` : receivedAt(m)}</small></span></button>; })}{!visible.length && <div className="m2-empty">{queues[view].empty}</div>}</div><div className="m2-pagination" aria-label="邮件分页"><button className="m2-text-btn" disabled={pageByView[view] <= 1} onClick={() => setPageByView((current) => ({ ...current, [view]: current[view] - 1 }))}>上一页</button><span>第 {pageByView[view]} / {Math.max(1, Math.ceil(totals[view] / PAGE_SIZE))} 页</span><button className="m2-text-btn" disabled={pageByView[view] >= Math.max(1, Math.ceil(totals[view] / PAGE_SIZE))} onClick={() => setPageByView((current) => ({ ...current, [view]: current[view] + 1 }))}>下一页</button></div><section className="m2-ai"><span><IconSparkles size={20}/></span><div><strong>AI 建议</strong><p>{view === "archive" ? (totals.archive ? `已归档 ${totals.archive} 封邮件，AI 当时已完成分类并由你关闭。如需重新处理，可在详情页点击"恢复为待处理"。` : "暂无已归档邮件。") : high ? `共识别到 ${high} 封 P0 邮件，建议优先处理今天到期或已逾期的事项。` : data.uncertain.length ? `还有 ${data.uncertain.length} 封邮件等待人工确认，请先完成分类，再按截止时间处理行动队列。` : "当前没有 P0 高优先级邮件，可按截止时间处理队列。"}</p></div></section></section>
      <aside className={`m2-detail ${isClosedStatus(selected?.status) ? "m2-detail--archive" : ""}`}>{selected ? <><div className="m2-detail__meta"><span className={`m2-priority m2-priority--${(selected.priority || "P2").toLowerCase()}${isClosedStatus(selected.status) ? " m2-priority--archived" : ""}`}>{selected.priority || "P2"}</span><span>{isClosedStatus(selected.status) ? closedStatusLabel(selected.status) : selected.queue === "action" ? "需要行动" : queues[selected.queue]?.label}</span>{isClosedStatus(selected.status) && (() => { const t = closedAtOf(selected); return <span className="m2-detail__closed"><IconArchive size={13}/>{closedStatusLabel(selected.status)}{t ? ` · ${t.toLocaleString("zh-CN", { month: "numeric", day: "numeric", hour: "2-digit", minute: "2-digit" })}` : ""}</span>; })()}</div><h2>{selected.subject || "（无主题）"}</h2><p className="m2-detail__from">{selected.sender || "未知发件人"} · {receivedAt(selected)}</p><hr/>{isClosedStatus(selected.status) ? <><h3><IconCircleCheck size={18}/>分类回顾</h3><div className="m2-recommend m2-recommend--archive"><strong>AI 当时判断：{selected.actionText || "无需特别处理"}</strong><p>{selected.priorityReason || "AI 已依据邮件内容完成分类。"}该邮件已{closedStatusLabel(selected.status)}，不再按风险提醒。</p></div></> : <><h3><IconSparkles size={18}/>AI 行动建议</h3><div className="m2-recommend"><strong>{selected.actionText || "请阅读邮件并确认下一步"}</strong><p>{selected.priorityReason || "AI 已依据邮件内容和截止时间完成分类。"}</p></div></>}<h3><IconFileText size={18}/>邮件摘要</h3><p className="m2-summary">{selected.summary || "AI 暂未生成摘要。"}</p>{selected.bodyText && <details className="m2-body-toggle"><summary>查看完整邮件正文（{selected.bodyText.length} 字）</summary><pre className="m2-body">{selected.bodyText}</pre></details>}{selected.dueAt && <div className={`m2-deadline ${isClosedStatus(selected.status) ? "m2-deadline--archive" : ""}`}><IconCalendarDue size={17}/><div><span>{isClosedStatus(selected.status) ? "原截止时间" : "截止时间"}</span><strong>{dueInfo(selected).detail}</strong></div></div>}<div className="m2-detail__actions">{selected.queue === "uncertain" && selected.status === "open" && <><button className="m2-btn" disabled={pending} onClick={()=>run(()=>outlookApi.correct(selected.id,{queue:"action"}))}>设为需要行动</button><button className="m2-btn m2-btn--quiet" disabled={pending} onClick={()=>run(()=>outlookApi.correct(selected.id,{queue:"informational"}))}>设为仅供知晓</button></>}{selected.queue === "action" && selected.status === "open" && <button className="m2-btn" disabled={pending} onClick={() => convert(selected)}><IconListCheck size={16}/>转为待办</button>}{selected.queue === "action" && selected.status === "open" && <button className="m2-btn m2-btn--quiet" onClick={() => setDrafting(selected)}><IconSparkles size={16}/>生成回复草稿</button>}<button className="m2-btn m2-btn--quiet" onClick={() => open(selected)}><IconArrowUpRight size={16}/>打开原邮件</button>{selected.status === "open" && <button className="m2-text-btn" onClick={() => run(() => outlookApi.setStatus(selected.id, selected.queue === "action" ? "ignored" : "processed"))}><IconCheck size={15}/>{selected.queue === "action" ? "标为无需处理" : "标记已处理"}</button>}{isClosedStatus(selected.status) && <button className="m2-text-btn" onClick={() => run(() => outlookApi.setStatus(selected.id, "open"))}><IconRefresh size={15}/>恢复为待处理</button>}</div></> : <div className="m2-detail__empty"><IconMail size={32}/><p>从左侧选择一封邮件查看详情</p></div>}</aside></div>
