...
export default function AiHotPage() {
  ...
  const refresh = useCallback(async (force = false) => {
    setRefreshing(true);
    try {
      const data = await api.get(`/ai-hot${force ? "?force=1" : ""}`);
      setState({
        data,
        status: data?.status === "stale" ? "stale" : "live",
        error: null,
      });
    } catch (e) {
      setState({ data: null, status: "unavailable", error: e.message });
    } finally {
      setRefreshing(false);
    }
  }, []);
    ...
    <div className="page page--daily-hot">
      ...
      {loading ? (
        ...
        <div className="error-note daily-hot-unavailable">
          ...
          <p>{state.error || data?.error?.message || "本地数据服务或外部来源不可用。"}</p>
