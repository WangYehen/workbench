...
export default function TeamPage(){
  ...
  return <div className="workspace-page"><header className="workspace-head"><div><h1>团队态势</h1><p>查看缺交、阻塞、待决策，并下钻阅读每位成员的完整日志</p></div><div className="team-head-actions"><DateNav date={date} onChange={setDate}/><button className="btn primary" onClick={syncNow} disabled={syncing}><IconRefresh size={16}/>{syncing?"同步中…":"立即同步团队日志"}</button></div></header>
    ...
    <div className="member-grid">{members.map((member)=><article className={`member-card ${member.signalScore>0?"member-card--alert":""}`} key={member.user_id}><div className="member-card__head"><div><h3>{member.name}</h3><p>{member.dept_name||"未设置部门"}</p></div>{!member.submitted&&<span className="pill amber">未提交</span>}</div><p>{member.submitted?`已提交 ${member.reportCount} 份`:"当日无日志"} · 阻塞 {member.blockers.length} · 待决策 {member.review.length}</p><button className="member-card__detail" onClick={()=>setSelected(member)}><IconFileText size={15}/>{member.submitted?"查看日志详情":"查看成员状态"}</button></article>)}{!members.length&&<div className="empty-state"><IconAlertTriangle size={20}/>当前筛选条件下没有成员。</div>}</div></>}
