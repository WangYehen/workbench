...
- DWS Agent：通过自然语言查询钉钉日报、待办和日程；涉及写入工作台或钉钉的操作会先展示预览，确认后执行。
- AI 任务队列：统一处理邮件分类、钉钉消息研判、团队分析、日报/周报生成，并记录任务状态、重试、Token 用量和 Prompt 版本。
...
本地技能目录、浏览器验证产物和临时探测文件不会提交到版本库，包括 `.agents/`、`.artifacts/`、`.claude/`、`.playwright-cli/` 和 `.tmp-probe/`。
...
DWS Agent 接口前缀为 `/api/dws-agent`，支持会话管理、SSE 流式执行、真实数据查询，以及带幂等键的确认写入。
