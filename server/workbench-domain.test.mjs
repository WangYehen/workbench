...
import { localDateString, localTimeString, resolveDateKey } from "./local-date.mjs";
...
CREATE TABLE calendars(id TEXT PRIMARY KEY,source TEXT,title TEXT,start_at TEXT,end_at TEXT,location TEXT,organizer TEXT,day TEXT);
`);return db}
...
test("UTC 日程时间按上海时区展示",()=>{assert.equal(localTimeString("2026-08-27T07:00:00.000Z"),"15:00");const db=memoryDb();db.prepare("INSERT INTO calendars VALUES(?,?,?,?,?,?,?,?)").run("m1","dingtalk","测试","2026-08-27T07:00:00.000Z","2026-08-27T08:00:00.000Z","会议室 A","Charles","2026-08-27");const dashboard=buildDashboard(db,"2026-08-27");assert.equal(dashboard.meetings[0].start,"15:00");assert.equal(dashboard.meetings[0].end,"16:00");db.close()});
